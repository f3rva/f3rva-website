...
import DayArchives from './DayArchives';
...
describe('DayArchives Page', () => {
  ...
  const renderComponent = (year = '2024', month = '03', day = '15') => {
    return render(
      <MemoryRouter initialEntries={[`/archives/${year}/${month}/${day}`]}>
        <Routes>
          <Route path="/archives/:year/:month/:day" element={<DayArchives />} />
        </Routes>
      </MemoryRouter>
    );
  };
  ...
  it('shows loading state initially', () => {
    ...
    renderComponent();
    expect(screen.getByText(/Loading day archives/i)).toBeInTheDocument();
  });
  ...
  it('renders posts when API returns data', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining('year=2024&month=03&day=15'),
      expect.any(Object)
    );
  });
  ...
  it('handles API errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Loading day archives...')).not.toBeInTheDocument();
    });

    expect(screen.getByText(/Error loading day archives/i)).toBeInTheDocument();
